...
\title{Семинар \textnumero 5 по курсу <<Электромагнитная совместимость
...
\begin{equation}
 U(\mbox{дБмкВ})=20\lg\frac{U}{U_0} \label{eq:dbUv}
\end{equation}
...
\begin{enumerate}
 \item Собрать в Qucs схему на рис.\ref{thyr-qucs}
 \item Смоделировать переходный процесс в схеме. Длительность моделирования 20 
мс, количество точек расчёта --- 10000
 \item Рассчитать спектр напряжения помех, используя функцию 
\verb|Time2Freq()|. Описание функции приведено во встроенной справочной 
системе (запускается по F1). Напряжение помех снимается с узла \verb|w1|.
 \item Рассчитать напряжение помех в дБмкВ, используя формулу (\ref{eq:dbUv}) и 
функцию \verb|dB()|.
 \item В отчёт включить спектр помех и осциллограммы тока нагрузки. Выполнить 
